...
class FillingsClass:
    def __init__(self, root):
        ...
        self.root.title("Fillings Clinic Management")
        ...
        lbl_title = Label(
            header_frame, 
            text="🩺 Fillings & Treatments Management", 
        ...
        self.fillings_table = ttk.Treeview(
            table_frame,
        ...
        scrollx.config(command=self.fillings_table.xview)
        scrolly.config(command=self.fillings_table.yview)
        ...
        for col_id, heading_text, width in headers:
            self.fillings_table.heading(col_id, text=heading_text)
            self.fillings_table.column(col_id, width=width, anchor=CENTER)
        
        self.fillings_table["show"] = "headings"
        self.fillings_table.pack(fill=BOTH, expand=1)
        
        self.show()
        self.fillings_table.bind("<ButtonRelease-1>", self.get_data)
    ...
    def reco_id(self):
        ...
        try:
            cur.execute("""
                ...
                UPDATE fillings
                SET id = (SELECT row_num FROM cte WHERE cte.id = fillings.id);
            """)
            con.commit()
    ...
    def add(self):
        ...
        if self.var_name.get() == "" or self.var_gender.get() == "Select" or self.var_age.get() == "" or self.var_date.get() == "" or self.var_state.get() == "" or self.var_price.get() == "" or self.var_contact.get() == "" or self.var_address.get() == "":
            ...
            try:
                cur.execute("INSERT INTO fillings (name, gender, age, date, state, price, contact, address) VALUES (?, ?, ?, ?, ?, ?, ?, ?)", (
                    self.var_name.get(),
                    self.var_gender.get(),
                    self.var_age.get(),
                    self.var_date.get(),
                ...
                con.commit()
                messagebox.showinfo("Success", "Fillings record added successfully", parent=self.root)
                self.reco_id()
                self.show()
                self.clear()
            except Exception as ex:
    ...
    def show(self):
        ...
        cur = con.cursor()
        try:
            cur.execute("""CREATE TABLE IF NOT EXISTS fillings (
                            ...
                            contact TEXT,
                            address TEXT)""")
            cur.execute("SELECT id, name, gender, age, date, state, price, contact, address FROM fillings")
            rows = cur.fetchall()
            self.fillings_table.delete(*self.fillings_table.get_children())
             
            for row in rows:
                self.fillings_table.insert('', END, values=(row[1], row[2], row[3], row[4], row[5], row[6], row[7], row[8], row[0]))
        except Exception as ex:
            messagebox.showerror("Error", f"Error in fillings: {str(ex)}")
        finally:
            con.close()
    ...
    def get_data(self, ev):
        f = self.fillings_table.focus()
        content = (self.fillings_table.item(f))
        row = content['values']
        if row:
    ...
    def update(self):
        ...
        if self.var_id.get() == "":
            ...
            try:
                cur.execute("update fillings set name=?, gender=?, age=?, date=?, state=?, price=?, contact=?, address=? where id=?", (
                    self.var_name.get(),
                    self.var_gender.get(),
                    self.var_age.get(),
                    self.var_date.get(),
    ...
    def delete(self):
        ...
        try:
            ...
            if op == True:
                cur.execute("delete from fillings where id=?", (self.var_id.get(),))
                con.commit()
                self.reco_id()
    ...
    def search(self):
        ...
        try:
            if self.var_searchby.get() == "Select":
                ...
                cur.execute("select * from fillings where " + self.var_searchby.get() + " LIKE '%" + self.var_searchtxt.get() + "%'")
                rows = cur.fetchall()
                if len(rows) != 0:
                    self.fillings_table.delete(*self.fillings_table.get_children())
                    for row in rows:
