...
class XraysClass:
    def __init__(self, root):
        ...
        self.root.title("X-Rays Management")
        ...
        for col_id, heading_text, width in headers:
            self.xray_table.heading(col_id, text=heading_text)
            self.xray_table.column(col_id, width=width, anchor=CENTER)
    ...
    def reco_id(self):
        ...
        try:
            cur.execute("""
                ...
                UPDATE xrays
                SET id = (SELECT row_num FROM cte WHERE cte.id = xrays.id);
            """)
            con.commit()
    ...
    def add(self):
        ...
        if self.var_name.get() == "" or self.var_gender.get() == "Select" or self.var_age.get() == "" or self.var_date.get() == "" or self.var_state.get() == "" or self.var_price.get() == "" or self.var_contact.get() == "" or self.var_address.get() == "":
            ...
            try:
                cur.execute("INSERT INTO xrays (name, gender, age, date, state, price, contact, address) VALUES (?, ?, ?, ?, ?, ?, ?, ?)", (
                    self.var_name.get(),
                    self.var_gender.get(),
                    self.var_age.get(),
                    self.var_date.get(),
                ...
                con.commit()
                messagebox.showinfo("Success", "X-ray record added successfully", parent=self.root)
                self.reco_id()
                self.show()
                self.clear()
            except Exception as ex:
    ...
    def show(self):
        ...
        cur = con.cursor()
        try:
            cur.execute("""CREATE TABLE IF NOT EXISTS xrays (
                            ...
                            contact TEXT,
                            address TEXT)""")
            cur.execute("SELECT id, name, gender, age, date, state, price, contact, address FROM xrays")
            rows = cur.fetchall()
            self.xray_table.delete(*self.xray_table.get_children())
             
            for row in rows:
                self.xray_table.insert('', END, values=(row[1], row[2], row[3], row[4], row[5], row[6], row[7], row[8], row[0]))
        except Exception as ex:
            messagebox.showerror("Error", f"Error in xrays: {str(ex)}")
        finally:
            con.close()
    ...
    def update(self):
        ...
        if self.var_id.get() == "":
            ...
            try:
                cur.execute("update xrays set name=?, gender=?, age=?, date=?, state=?, price=?, contact=?, address=? where id=?", (
                    self.var_name.get(),
                    self.var_gender.get(),
                    self.var_age.get(),
                    self.var_date.get(),
                ...
                con.commit()
                self.reco_id()
                messagebox.showinfo("Success", "X-ray record updated successfully", parent=self.root)
                self.show()
                self.clear()
            except Exception as ex:
                messagebox.showerror("Error", f"Error: {str(ex)}", parent=self.root)
    ...
    def delete(self):
        ...
        try:
            ...
            if op == True:
                cur.execute("delete from xrays where id=?", (self.var_id.get(),))
                con.commit()
                self.reco_id()
